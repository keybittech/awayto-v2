...
import { IGroup, IGroupRole, DbError, IUserProfile, IGroupState, IRole, IGroupActionTypes, asyncForEach, utcNowString, IPrompts, ApiErrorResponse, ApiInternalError } from 'awayto/core';
    ...
    cmnd: async (props) => {
      ...
      try {
        ...
        for (const roleId in roles) {
          ...
          const { id: kcSubgroupId } = await keycloak.groups.setOrCreateChild({ id: kcGroupExternalId }, { name: roleName });
          await props.db.query(`
        ...
        if (diffs.length) {
          ...
          if (externalGroup?.subGroups) {
            const roleNames = (await props.db.query<IRole>(`
              ...
              WHERE id = ANY($1::uuid[])
            `, [diffs.map(r => r.roleId)])).rows.map(r => r.name) as string[];
...

  {
    ...
    cmnd: async (props) => {
      ...
      for (const userId in users) {
        ...
        try {
          const { id: keycloakUserId } = await keycloak.users.create({
            email,
            username: email,
            enabled: true,
          ...
          await keycloak.users.sendVerifyEmail({ id: keycloakUserId });
        } catch (error) {
