...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-'EOSQL'
  ...
  JOIN dbtable_schema.group_schedules gs ON gs.group_id = g.id
  JOIN dbtable_schema.group_user_schedules gus ON gus.group_schedule_id = gs.schedule_id
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE
      dss.group_schedule_id = gs.schedule_id
  ) as dcsc ON true
  WHERE dcsc.slots IS NOT NULL
  GROUP BY gs.schedule_id, sbss.services, scheduleTimeUnit.name, bracketTimeUnit.name, slotTimeUnit.name;
