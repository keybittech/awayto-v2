...
\ c sysmaindb CREATE
OR REPLACE VIEW enabled_users AS
SELECT
  u.id,
...
CREATE
OR REPLACE VIEW enabled_groups AS
SELECT
  id,
...
CREATE
OR REPLACE VIEW enabled_uuid_groups AS
SELECT
  id,
...
CREATE
OR REPLACE VIEW enabled_roles AS
SELECT
  id,
  name,
  row_number() OVER () as row
...
CREATE
OR REPLACE VIEW enabled_uuid_roles AS
SELECT
  id,
  parent_uuid as "parentUuid",
  role_id as "roleId",
...
CREATE
OR REPLACE VIEW enabled_file_types AS
SELECT
  id,
  name,
  row_number() OVER () as row
...
CREATE
OR REPLACE VIEW enabled_files AS
SELECT
  f.id,
  f.uuid,
  f.name,
...
CREATE
OR REPLACE VIEW enabled_uuid_files AS
SELECT
  uf.id,
...
CREATE
OR REPLACE VIEW enabled_uuid_notes AS
SELECT
  un.id,
...
CREATE
OR REPLACE VIEW enabled_groups_ext AS
SELECT
  eg.*,
  ug.users,
  rls.* as roles
...
CREATE
OR REPLACE VIEW enabled_users_ext AS
SELECT
  u.*,
  grps.*,
  rols.*
